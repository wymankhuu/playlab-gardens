...
// ---- Admin Mode (lightweight globals — drawer.js uses these) ----
...
// ---- Star helpers (lightweight — used by appCardHTML before drawer loads) ----
const _starCountCache = {};

function isStarred(appId) {
  try {
    const stars = JSON.parse(localStorage.getItem('playlab-gardens-stars')) || {};
    return !!stars[appId];
  } catch { return false; }
}
